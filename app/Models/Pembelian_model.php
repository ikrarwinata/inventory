...
class Pembelian_model extends Model
{
    ...
    {
        return $this->orderBy($this->table . "." . $this->columnIndex, $this->order)->where($key, $val)->findAll();
    }

    public function total_rows($arr = NULL)
    {
        if ($arr == NULL) {
            return $this->orderBy($this->table . "." . $this->columnIndex, $this->order)->countAllResults();
        }else{
