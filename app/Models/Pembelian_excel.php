...
class Pembelian_excel extends Model
{
    ...
    protected $allowedFields = ['id', 'id_master', 'quantity', 'harga', 'timestamps', 'id_vendor', 'username'];
    ...
    public function export()
    {
        ...
        ->setTitle('Pembelian Data')
        ->setSubject('Pembelian Data')
        ->setDescription('Pembelian Data '.date("Y"))
        ->setKeywords('Pembelian')
        ->setCategory('Pembelian');

        // subtitle on row 3
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A3', 'Pembelian Data'.date("Y"));
        ...
        ->setCellValue('E' . $startRowHeader, 'ID Vendor')
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', "Pembelian Data");
        $spreadsheet->getActiveSheet()->getStyle('A1:'.$highestColumn.'1')->applyFromArray($this->headerStyle);

        $startRowBody = $startRowHeader+1;
        $index = 0;
        $data_pembelian = $this->orderBy($this->columnIndex, $this->order)->findAll();
        foreach ($data_pembelian as $key => $pembelian) {
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('A' . $startRowBody, ++$index);
            if (isset($pembelian->id))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('B' . $startRowBody, $pembelian->id);
            if (isset($pembelian->id_master))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('C' . $startRowBody, $pembelian->id_master);
            if (isset($pembelian->timestamps))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('D' . $startRowBody, date("d-m-Y", $pembelian->timestamps));
            if (isset($pembelian->id_vendor))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('E' . $startRowBody, $pembelian->id_vendor);
            if (isset($pembelian->quantity))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('F' . $startRowBody, $pembelian->quantity);
            if (isset($pembelian->harga))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('G' . $startRowBody, $pembelian->harga);
            $spreadsheet->setActiveSheetIndex(0)->setCellValue('H' . $startRowBody, '=(F' . $startRowBody . '*G' . $startRowBody . ')');
            if (isset($pembelian->username))
                $spreadsheet->setActiveSheetIndex(0)->setCellValue('I' . $startRowBody, $pembelian->username);
            $startRowBody++;
        };
        ...
        $spreadsheet->getActiveSheet()->setTitle('Pembelian Data '.date('Y'));
        $spreadsheet->setActiveSheetIndex(0);
        // Redirect output to a client’s web browser (Xlsx)
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Pembelian '.date("Y").'.xlsx"');
